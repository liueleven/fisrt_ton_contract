...
// import cell from "../build/counter.cell"; // this is the interface class from step 7
import dotenv from "dotenv"
dotenv.config()
export async function run() {
    // initialize ton rpc client on testnet
    console.log("initialize ton rpc client on testnet");
    const endpoint = await getHttpEndpoint({ network: "testnet" });
    const client = new TonClient({ endpoint });

    // prepare Counter's initial code and data cells for deployment
    const counterCode = Cell.fromBoc(fs.readFileSync("/Users/liuziqing/work/learn/blockchain/ton/contract/counter/counter/build/counter.cell"))[0]; // compilation output from step 6
    ...
    if (!mnemonic) {
        return console.log("Please set MNEMONIC in .env file");
    }
